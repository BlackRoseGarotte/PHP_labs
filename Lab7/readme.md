# Лабораторная работа №7. Разделение логики и представления. Шаблонизация
    ...
    <tbody>
        {% for book in books %}
            <tr>
                <td>{{ book.title }}</td>
                <td>{{ book.author }}</td>
                <!-- Применение кастомного фильтра (см. Шаг 4) -->
...
**Преимущества Twig:**
- Автоматическое экранирование переменных снижает риск человеческой ошибки и повышает безопасность приложения по умолчанию;
- Синтаксис шаблонов отделён от PHP-кода, что делает вёрстку чище и позволяет верстальщикам работать с шаблонами без глубоких знаний программирования;
- Встроенная система наследования шаблонов (`{% extends %}`, `{% block %}`) упрощает создание единого макета для всего сайта;
- Поддержка кэширования скомпилированных шаблонов ускоряет выполнение приложения в продакшене;
...
**Интерфейс в режиме нативного PHP и интерфейс в режиме Twig визуально не отличаются.**
