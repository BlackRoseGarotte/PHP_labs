...
**2. Логика расписания**  
Расписание для каждого сотрудника определяется с помощью отдельных блоков условного оператора `if`. Проверка дней недели выполняется через логическое `ИЛИ (||)`, объединяющее нужные числовые значения. В зависимости от результата проверки, переменной сразу присваивается время работы или текст "Нерабочий день".
